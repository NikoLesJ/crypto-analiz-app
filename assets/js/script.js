...
async function fetchAndDisplay(url, idsToTogle, displayFunction, tabName = null, localKey) {
    idsToTogle.forEach(id => {
    ...
    if(localData) {
        idsToTogle.forEach(id => toggleSpinner(id, `${id}-spinner`, false));
...
function displayTrendNfts(nfts){
    ...
    nfts.forEach(nft => {
        ...
        row.innerHTML = `
            ...
            <td class="${parseFloat(nft.data.floor_price_in_usd_24h_percentage_change) >= 0 ?
                 'green' : 'red'}">${parseFloat(nft.data.floor_price_in_usd_24h_percentage_change).toFixed(2)}%</td>
...
function displayExchanges(data) {
    ...
    const table = createTable(['Rank', 'Exchanges', 'Trust Score', '24h Trade', '24h Trade (Normal)', 'Country', 'Website', 'Year'], 1);
...
function displayCategories(data) {
    ...
    data.forEach(category => {
        ...
        row.innerHTML = `
            ...
            <td>$${category.market_cap ? category.market_cap.toLocaleString(undefined, {
                minimumFractionDigits: 3, maximumFractionDigits: 3
            }) : "N/A"}</td>
            ...
            <td>$${category.volume_24h ? category.volume_24h.toLocaleString(undefined, {
                minimumFractionDigits: 3, maximumFractionDigits: 3
            }) : "N/A"}</td>
        `;
        table.appendChild(row);
