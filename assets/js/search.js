...
const exchangeList = document.getElementById('exchanges-list');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(query) {
        fetchSearchResult(query, [coinsList, exchangeList, nftsList]);
    } else {
...
function fetchSearchResult(param, idsToToggle) {
    ...
    searchHeading.innerText = `Search resuls for "${param}"`;
...
function exchangesResult(exchanges) {
    ...
    exchanges.forEach(ex => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td class="name-column"><img src="${ex.thumb}" alt="${ex.mame}">${ex.mame}</td>
